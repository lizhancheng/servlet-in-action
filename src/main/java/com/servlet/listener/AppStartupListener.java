...
public class AppStartupListener implements ServletContextListener {
 ...
 // 模拟应用配置
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        initializeAppConfig(sce);
        
        // 3. 启动后台任务（如定时清理、监控等）
        startBackgroundTasks(sce);
        
        // 4. 设置应用级别的属性，供所有Servlet使用
        sce.getServletContext().setAttribute("appName", APP_NAME);
    ...
    public void contextDestroyed(ServletContextEvent sce) {
        ...
        sce.getServletContext().removeAttribute("version");
     ...
     * 模拟初始化应用配置
     */
    private void initializeAppConfig(ServletContextEvent sce) {
        // 这里可以是读取配置文件、数据库连接池初始化等操作
        System.out.println("📋 初始化应用配置...");
        
        // 模拟设置一些配置参数
        sce.getServletContext().setAttribute("maxUsers", 1000);
        sce.getServletContext().setAttribute("sessionTimeout", 1800); // 秒
     ...
     * 启动后台任务
     */
    private void startBackgroundTasks(ServletContextEvent sce) {
        System.out.println("🔄 启动后台任务...");
        
        // 创建定时任务线程池
        scheduler = Executors.newScheduledThreadPool(1);
        
        // 模拟一个定时任务：每分钟输出一次应用状态
        scheduler.scheduleAtFixedRate(() -> {
            System.out.println("[后台任务] 应用运行中... 当前时间: " + 
                LocalDateTime.now().toString().substring(11, 19));
        }, 1, 1, TimeUnit.MINUTES); // 延迟1分钟开始，之后每分钟执行一次
